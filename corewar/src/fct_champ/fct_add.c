...
int fct_add(vm_t *vm)
{
    ...
    size_t third_arg = get_arg_value(vm, VM_PROCESS_ACTU->pos_x + 2 + T_REG
    + T_REG,VM_PROCESS_ACTU->pos_y, T_REG);

    if (arg_size_one != T_REG || arg_size_two != T_REG || arg_size_three !=
    T_REG)
        return 84;
