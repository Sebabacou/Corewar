...
int fct_ld_ind(vm_t *vm, size_t size_arg_one)
{
    ...
    if (id_reg > REG_NUMBER)
        return 84;
    VM_PROCESS_ACTU->reg[id_reg] = get_arg_value(vm, temp + (one % IDX_MOD),
    0, REG_SIZE);
    VM_PROCESS_ACTU->carry = VM_PROCESS_ACTU->reg[id_reg];
    return 42;
}
...
int fct_ld_tdir(vm_t *vm, size_t size_arg_one)
{
    ...
    if (id_reg > REG_NUMBER)
        return 84;
    VM_PROCESS_ACTU->reg[id_reg] = get_arg_value(vm, temp + (one
    % IDX_MOD), 0, REG_SIZE);
    VM_PROCESS_ACTU->carry = VM_PROCESS_ACTU->reg[id_reg];
    return 42;
}
